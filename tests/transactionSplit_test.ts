...
import { Bearer, Currency, PaystackClient, TxSplit, type TransactionSplitCreatePayload, type UpdateTxSplitPayload } from "../mod.ts";
...
describe("PaystackClient.transactionSplit", () => {
  ...
  it("PaystackClient.splits.create", async () => {
    const payload: TransactionSplitCreatePayload = {
      ...
      subaccounts: [{subaccount: 'ACCT_iw34h1ss4p1luyd', share: 50},{subaccount: 'ACCT_l6nz8ofjywrc66k',share: 50}],
  ...
  it("PaystackClient.splits.addOrUpdate", async () => {
    const response = await client.splits.addOrUpdate('2880355', 'ACCT_iw34h1ss4p1luyd',30)
    assertEquals(response.statusCode,HttpStatusCode.Ok)
  ...
  it("PaystackClient.splits.remove", async () => {
    ...
    const response = await client.splits.remove('2880355','ACCT_ypicscilcimszal')
    assertEquals(response.statusCode, HttpStatusCode.Ok)
