...
import { Currency, PaystackClient, PaystackClientError, type CreateProductPayload, type UpdateProductPayload } from "../mod.ts";
...
describe("PaystackClient.products", () => {
  ...
  it("PaystackClient.products.update", async () => {
    const payload: UpdateProductPayload = {name: 'Test product', description: 'A test product',price: 500_000_000, currency: Currency.NGN}
    const err = await assertRejects(() => client.products.update("PROD_0tbswmrz56osyxr",payload))
