...
import { Interval, PaystackClient, PaystackClientError, type CreatePlanPayload, type CreateSubscriptionPayload } from "../mod.ts";
...
describe("PaystackClient.subscriptions", () => {
  ...
  it("PaystackClient.subscriptions.getSubscriptions", async () => {
    const response = await client.subscriptions.getSubscriptions({perPage: 2})
    assertEquals(response.statusCode, HttpStatusCode.Ok)
    assertEquals(response.message, 'Subscriptions retrieved')
  });

  it("PaystackClient.subscriptions.getSubscription",async () => {
    const response = await client.subscriptions.getSubscription('SUB_bbd8ws1ypay28cg')
    assertEquals(response.statusCode, HttpStatusCode.Ok)
    assertEquals(response.message, 'Subscription retrieved successfully')
  });

  it("PaystackClient.subscriptions.enable", async () => {
    const err =await assertRejects(() => client.subscriptions.enable('SUB_ievha8l1igtpuy1','5qzqh94hzndycwu'))
    ...
    assertEquals(data.message, 'Subscription with code not found or already active')
    assertEquals(data.type, 'validation_error')
    assertEquals(data.code, 'not_found')
  });

  it("PaystackClient.subscriptions.disable", async () => {
    const err = await assertRejects(() => client.subscriptions.disable('SUB_28fg2bcaqhg73l0','ozmiw6tukx4x31m'))
    assertInstanceOf(err, PaystackClientError)
    type DataOfInterest = {status: boolean; message: string; type: string; code: string}
    const data = (err.wrappedError as AxiosError).response?.data as DataOfInterest
    assertEquals(data.status, false)
    assertEquals(data.message, 'Subscription with code not found or already inactive')
    assertEquals(data.type, 'validation_error')
    assertEquals(data.code, 'not_found')
  });

  it("PaystackClient.subscriptions.getUpdateLink", async () => {
    const response = await client.subscriptions.getUpdateLink('SUB_r2aqxnxzob1vvlf')
    assertEquals(response.statusCode, HttpStatusCode.Ok)
  ...
  it.skip("PaystackClient.subscriptions.sendUpdateLink", async() => {
    ...
    const response = await client.subscriptions.sendUpdateLink('SUB_r2aqxnxzob1vvlf')
    assertEquals(response.statusCode, HttpStatusCode.Ok)
