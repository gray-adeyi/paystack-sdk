...
import { CreatePaymentRequestPayload, GetPaymentRequestsOptions } from "../types/clients/paymentRequest.ts";
...
export default class PaymentRequestClient {
  ...
  sendNotification(idOrCode: string){
    return this.client.call(`/paymentrequest/notify/${idOrCode}`, HTTPMethod.POST)
  }
  ...
  finalize(idOrCode: string){
    return this.client.call(`/paymentrequest/finalize/${idOrCode}`, HTTPMethod.GET)
  }

  update(idOrCode: string, payload: UpdatePaymentRequestPayload){
    return this.client.call(`/paymentrequest/${idOrCode}`, HTTPMethod.PUT, payload)
  }
}